...
# Documentación para el proyecto
 ...
 Una de las áreas de oportunidades que se tuvo en el proyecto no se alcanzo hacer el código de consultas. Que son 3 tipos de consultas, el primero seria la consulta de todos los participantes que sean registrados esto seria por medio de GET, donde se pedirá Id_Participante, este se genera de manera automática al momento de hacer las capturas de los participantes, esto. Al guardar el registro muestra el numero de participante que seria el Id para que con este pueda consultar hacer las consultas. Se tendría que hacer una función con GET donde pueda validar los datos. Esto seria que si es un dato valido el que esta ingresando, si no es valido manda un código que existe, el otro mensaje que arrojaría la función cuando la búsqueda fue exitosa.
 La consulta que se podrá hacer es el de los eventos que existen  mostraría el nombre del evento, distancia que es del evento y fecha que se va hacer el evento. También va llevar la validación de los datos que se estén introduciendo. En caso que los datos son erróneos manda un mensaje que "no son datos validos". Esta consulta también seria por medio de un Id. de eventos.
 Se hará también la consulta donde por medio del Id_Participante nos pueda mostrar a que eventos están inscritos y cuando serian los eventos.
